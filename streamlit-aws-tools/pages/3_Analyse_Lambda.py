...
# ──────────────────────────────────────────────────────────────────────────────
# Selected Lambdas from Discovery
# ──────────────────────────────────────────────────────────────────────────────
selected_lambdas: List[str] = st.session_state.get("ad_selected_lambdas", [])
if not selected_lambdas:
    st.info("No Lambda functions selected yet. Go to **App Discovery** and select Lambdas.")
    st.stop()

# Show selected lambdas in a compact table
...
# NEW: get the latest event (timestamp + one message) per function efficiently
def get_latest_event_for_function(logs_client, function_name: str) -> Tuple[Optional[datetime], Optional[str], Optional[str], Optional[str]]:
    ...
    or (None, reason, log_group, None) if not found / error (reason contains 'No events' or error).
    """
    log_group = f"/aws/lambda/{function_name}"
    try:
        # Get the most recent stream by last event time
        ...
        # Get latest event from that stream
        ...
        latest = events[-1]  # even with startFromHead=False, SDK can return chronological; take last safely
...
with col_bulk:
    bulk_btn = st.button("Check Last Run (All Selected)", type="primary", use_container_width=True)
with col_clear:
    clear_bulk_btn = st.button("Clear Results", use_container_width=True)
...
# Single Lambda - quick latest check (kept from your original flow)
...
if is_etl_batch:
    ...
    with col_invoke:
        btn_invoke = st.button("🚀 Invoke test", type="primary", use_container_width=True)
    with col_refresh:
        btn_refresh = st.button("🔄 Fetch logs again for last invocation", use_container_width=True)
